...
if (isset($_GET['hapus_id'])) {
    ...
    $delete_sql = "DELETE FROM narapidana WHERE id = $hapus_id";
...
<link rel="stylesheet" href="css/datanarapidana.css">
...
<div class="data-table">   
    <h3>Data Narapidana</h3>

    <!-- Kontainer untuk tombol -->
    <div class="btn-container">
        <button onclick="printTable()" class="btn-print">Print Data</button>
        <a href="tambah_narapidana.php" class="btn-tambah">Tambah Narapidana</a>
    </div>
        ...
        <tbody>
            <?php if ($result && $result->num_rows > 0): ?>
                ...
                <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                        <td><?= $no++; ?></td>
                        <td><img src="uploads/<?= htmlspecialchars($row['foto']); ?>" alt="Foto"></td>
                        <td><?= htmlspecialchars($row['nama']); ?></td>
                        <td><?= htmlspecialchars($row['jenis_kelamin']); ?></td>
                        <td><?= htmlspecialchars($row['jenis_kejahatan']); ?></td>
                        <td><?= htmlspecialchars($row['Blok_Kamar_hunian']); ?></td>
                        <td><?= htmlspecialchars($row['tanggal_masuk']); ?></td>
                        <td><?= htmlspecialchars($row['setengah_hukuman']); ?> tahun</td>
                        <td><?= htmlspecialchars($row['total_hukuman']); ?> tahun</td>
    ...
    <div class="action-buttons">
        ...
        <a href="update_napi.php?id=<?= $row['id']; ?>" class="btn-ubah">
    <i class="fa-solid fa-pen" style="color: #ffffff;"></i> Ubah
            ...
            <?php else: ?>
                <tr>
                    <td colspan="10">Tidak ada data narapidana</td>
                </tr>
            <?php endif; ?>
...
function konfirmasiHapus(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = "data_narapidana.php?hapus_id=" + id;
        }
    });
}
</script>
